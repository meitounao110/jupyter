...
def FPN(config):
    ...
    return {
        ...
        'datadir': '/mnt/share1/layout_data/v0.3/data/all_walls',
    }
...
class dataloader(VisionDataset):
    def __init__(self,
        ...
        self.list_path1 = list_path1
        self.list_path2 = list_path2
        self.load_name = load_name
        self.resp_name = resp_name
        self.extensions = extensions
        # self.sample_files = make_dataset(root, extensions, is_valid_file)
        self.sample_files = make_dataset_list(root, list_path1, list_path2, extensions, is_valid_file,
                                              max_iters=max_iters)
    ...
    def __getitem__(self, index):
        ...
        if self.list_path2 is not None:
            labeled_idxs, unlabeled_idxs = self.is_dataset()
            if index in labeled_idxs:
                labys = LABEL
            elif index in unlabeled_idxs:
                labys = NO_LABEL
        else:
            labys = NO_LABEL
        return load, resp, labys
    ...
    def is_dataset(self):
        unlabeled_idxs = []  # 列表
        with open(self.list_path2, 'r') as rf:
            labels = rf.read().splitlines()
        for idx in range(len(self.sample_files)):  # 数据中的长度，排序，即有多少个数据
            path = self.sample_files[idx]  # 对其中每个数据得到路径
            filename = os.path.basename(path)  # 返回路径中文件名
            if filename in labels:  # 若该文件名在有标记的数据文件列表中
                labels.remove(filename)
            else:
        ...
        if len(labels) != 0:  # 如果有标签列表长度不为0
            ...
            some_missing = ', '.join(labels[:5])  #
            raise LookupError(message.format(len(labels), some_missing))
        labeled_idxs = sorted(set(range(len(self.sample_files))) - set(unlabeled_idxs))
        return labeled_idxs, unlabeled_idxs


def make_dataset_list(root_dir, list_path1, list_path2, extensions=None, is_valid_file=None, max_iters=None):
    ...
    with open(list_path1, 'r') as rf:  # 打开list_path并读取其中内容
        for line in rf.readlines():
            ...
            if is_valid_file(path):  # 在files中加入读取的数据路径
                files.append(path)
    if list_path2 is not None:
        with open(list_path2, 'r') as rf:  # 打开list_path并读取其中内容
            for line in rf.readlines():
                data_path = line.strip()  # 移除字符串头尾的制定字符，此处为空格
                path = os.path.join(root_dir, data_path)  # path表示每个数据的路径
                if is_valid_file(path):  # 在files中加入读取的数据路径
