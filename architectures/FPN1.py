...
from architectures.model_init import weights_init
...
class FPN_ResNet(nn.Module):
    def __init__(
        ...
        self.final_conv = nn.Conv2d(segmentation_channels,
                                    ...
                                    kernel_size=1,
                                    padding=0)
        self.gn = nn.GroupNorm(32, bef_final_channels)
        self.conv2 = nn.Conv2d(bef_final_channels, final_channels, kernel_size=1)

    def forward(self, x):
        #x = F.interpolate(x, scale_factor=self.final_upsampling, mode="nearest")
        x = self.backbone(x)
...
def test():
    net = FPN_ResNet18()
    print(net)
    fms = net(torch.randn(10, 1, 200, 200))
    print(fms)
    for fm in fms:
        print(fm.size())
